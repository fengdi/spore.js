// Spore.js 1.0.0
// author Tangoboy
// Based on https://github.com/fengdi/Class.js
...
(function(root, factory) {
    ...
    /**
     * @param {String} text   TemplateText
     * @param {[Object]} options {debug:1, variable:{}}
    ...
    var Template = function(text, options){
        ...
        options = options||{};
        options.debug = isFinite(options.debug) ? options.debug : 1;


        var debugCode = function (code, evaluate) {
            var tryPrefix = 'try{';
            var trySuffix = '}catch(e){<code>}';
            var debugMode = options.debug;
            if(debugMode == 0){
                return tryPrefix + code + trySuffix.replace(/<code>/,'');
            }else if(debugMode == 1){
                return code;
            }else if(debugMode == 2){
                var lineCount = evaluate.split('\n').length;
                var evalcode = escaper(evaluate);
                var range = lineCount>1 ? "["+(line-lineCount+1)+"~"+line+"]" : line;
                return tryPrefix + code + 
                    trySuffix.replace(/<code>/,
                        'throw "error: ["+e+"], line:'+range+', code:'+evalcode+'"');
            }
        };


        text.replace(/<%=([\s\S]+?)%>|<%([\s\S]+?)%>|$/g, function(match, interpolate, evaluate, offset){
            source += escaper(text.slice(index, offset));
